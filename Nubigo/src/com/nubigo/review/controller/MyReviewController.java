...
public class MyReviewController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("loginUser") == null) { // 로그인 X
			...
			response.sendRedirect(request.getContextPath() + "/reviewMain.re");
			
		}
		else {
			Member loginUser = (Member)session.getAttribute("loginUser");
			
			ArrayList<Review> rlist = new ReviewService().showMyReviews(loginUser.getMemberNo());
			...
			ArrayList<Review> list = new ReviewService().myReviewList(pi, loginUser.getMemberNo());
			request.setAttribute("pi", pi);
			request.setAttribute("list", list);		
			request.getRequestDispatcher("views/review/myReview.jsp").forward(request, response);
